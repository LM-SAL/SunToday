...
from suntoday.downloaders.jsoc import fetch_aia_fits, fetch_hmi_fits, fetch_synframe_fits, find_latest_pfss_time

# The PFSS anchor is the time every series (AIA, HMI m45s and the NRT
# synframe, which only retains ~60 days) has data for, so one timestamp
# gives a temporally matched test data set.
...
print(f"Fetching synframe FITS file {timestamp}...")
fetch_synframe_fits(timestamp)
